...
def main() :
    ...
    parser.add_option('-p', action="store_true", default=False, dest="isPhoton")
    ...
    if options.isMC:
        suffix = "mc"
    outFileName = "photonPlots_preCuts_%s.root"%(suffix)
    if options.debug:
        outFileName = "photonPlots_preCuts_%s_test.root"%(suffix)
    ...
    for event in fChain:
        ...
        if options.isMC:
            triggers = True
        if (triggers) and (ra2_nPhotonsIso>0 and ra2_Photon1Pt>50):
        ####Fill plots
